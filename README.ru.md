...
* **Легковесная:** Минимальные зависимости (только `uuid`, `proper-lockfile`).
...
WiseJSON DB использует всего две runtime-зависимости:

- [`uuid`](https://www.npmjs.com/package/uuid) — генерация уникальных идентификаторов
- [`proper-lockfile`](https://www.npmjs.com/package/proper-lockfile) — безопасное файловое блокирование, защита от гонок при работе из нескольких процессов

Обе зависимости ставятся автоматически при установке:

```bash
npm install wise-json-db
````

Если вы используете кастомную сборку или подключаете WiseJSON отдельно, убедитесь, что эти пакеты установлены:
...
**Безопасность многопроцессной работы достигается с помощью [proper-lockfile](https://www.npmjs.com/package/proper-lockfile).**
