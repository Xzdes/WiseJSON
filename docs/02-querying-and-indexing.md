...
*   **Поведение:** Если индекс для указанного поля не существует, метод ничего не сделает и не вызовет ошибку" также полностью соответствует обновленной логике (где теперь выводится WARN, а не ошибка),(Вместо ошибки будет выведено предупреждение в лог).
