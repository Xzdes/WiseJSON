...
*   **Поведение при ошибках:** Если при вставке хотя бы одного документа из массива возникает ошибка (например, нарушение уникального индекса из-за дублирующегося значения в одном из документов массива или конфликт с уже существующими данными), **вся операция `insertMany` будет отменена**, и ни один документ из этого вызова не будет вставлен. Будет выброшена ошибка.
